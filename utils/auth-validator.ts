...
import { JWKS } from "@passageidentity/passage-node/lib/cjs/types/JWKS";
...
const fetchJWKS = async (resetCache?: boolean): Promise<JWKS> => {
  ...
  try {
    const res = await fetch(`https://auth.passage.id/v1/apps/${process.env.NEXT_PUBLIC_PASSAGE_APP_ID}/.well-known/jwks.json`)
...
export const getUserFromAuthToken = async (token: string): Promise<string | null> => {
  try {
